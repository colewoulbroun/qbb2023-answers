...
def combine_bedgraphs(forward_fname, reverse_fname, chrom, chromstart, chromend, shift):
    ...
    # Adjust coverage for combined bedgraphs
    combined_bedgraph = combined_bedgraph[shift:] + [0] * shift
...
def calculate_background(control_bedgraph, chrom, chromstart, chromend, width):
    ...
    for i in range(chromend - chromstart):
        ...
        # Multiply the mean by 2 * width bases
        mean_value = mean_value * (2 * width)
...
def write_wiggle(pvalues, chrom, chromstart, fname):
    ...
    for i in pvalues:
        # Change values equal to 0 to an arbitrarily small number
        i = min(-numpy.log10(i) if i != 0 else -numpy.log10(1e-10), 50)
        print(i, file=output)
    output.close()
